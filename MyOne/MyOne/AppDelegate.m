...
//  MyOne
//
//  Created by zhenfei ren on 2019/1/17.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (![[UserDefaults objectForKey:MLBLastShowIntroduceVersionAndBuild] isEqualToString:versionAndBuild]) {
        self.window.rootViewController = [[MLBIntroduceViewController alloc] init];
    }else{
        [self showMainTabBarontrollers];
    }
    return YES;
}

-(void)showMainTabBarontrollers{
    self.window.rootViewController = [[MLBTabBarController alloc] init];
}
